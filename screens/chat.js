...
const ChatScreen = (props) => {
    ...
    setInterval(() => {
      const sendRequest = async () => {
        const response = await fetch(
          `http://192.168.1.17:5000/api/message/agriculteur/${auth.userId}`
        );
  ...
  const addMessage = async () => {
    ...
    const p = {
      ...
      idRecever: "61830b977c48820e480925cb",
      IdAgriculteur: auth.userId,
    };
    setMessages(messages.concat(p));

    let response = await fetch("http://192.168.1.17:5000/api/message/ajout", {
      method: "POST",
      headers: {
            ...
            messages.map((row) => (
              <View>
                {row.idSender === auth.userId
                  ? row.idRecever === "61830b977c48820e480925cb" && (
                      <View
                        style={
                          row.idSender === auth.userId
                            ? {
                  ...
                  : row.idSender === "61830b977c48820e480925cb" && (
                      <View
                        style={
                          row.idSender === auth.userId
                            ? {
